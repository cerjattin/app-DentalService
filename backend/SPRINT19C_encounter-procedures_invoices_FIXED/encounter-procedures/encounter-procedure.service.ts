...
export class EncounterProcedureService {
  ...
  async create(
    ...
    const created = await serializableTransaction(async (tx) => {
      ...
      const appointmentSvbContext = await tx.appointment.findFirst({
        where: {
          id: currentEncounter.appointment.id,
          organizationId: actor.organizationId,
        },
        select: {
          treatmentCase: {
      ...
      const procedure = await tx.encounterProcedure.create({
        data: {
          ...
          treatmentIdSnapshot:
            appointmentSvbContext.treatmentCase?.treatmentId ?? null,
          accidentFormNumberSnapshot:
            appointmentSvbContext.accidentCase?.accidentFormNumber ?? null,
          // NumberOfTreatments and Assistance intentionally remain unset here.
          // Their definitive SVB semantics are still an external contract blocker;
          // do not derive either value from quantity or invent a backend default.
          numberOfTreatmentsSnapshot: null,
          assistanceSnapshot: null,
          policlinicSnapshot:
            appointmentSvbContext.clinicLocation.policlinicCode ?? null,
